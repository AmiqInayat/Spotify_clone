...
router =APIRouter()
cloudinary.config( 
    cloud_name = "dccfcialt", 
    api_key = "899391793536667", 
    api_secret = "I2xqGXaGuGch6AZxD_aCFUSF9JM", # Click 'View API Keys' above to copy your API secret
    secure=True
)
...
def list_fav_songs(db: Session=Depends(get_db), 
    ...
    fav_songs = db.query(Favorite).filter(
        Favorite.user_id == user_id).options(
